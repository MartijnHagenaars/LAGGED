...
namespace LAG::Noise
{
	GeneratedData Noise::GenerateNoiseData(const Properties& noiseProps, const glm::vec2& position, const glm::vec2& size, int surfaceSubdivision)
	{
		...
		noiseData.m_Data.resize(static_cast<size_t>((scale.x) * (scale.y)));
		...
		if (size.x > 0 && size.y > 0)
		{
			...
			generator->GenUniformGrid2D(noiseData.m_Data.data(), static_cast<int>(position.x), static_cast<int>(position.y), static_cast<int>(scale.x), static_cast<int>(scale.y), noiseProps.m_Frequency * frequencyAdjustment, noiseProps.m_Seed);
	...
	Texture Noise::GeneratePreviewTexture(const Properties& noiseProperties, const glm::vec2& position, const glm::vec2& size, int surfaceSubdivision)
	{
		...
		texture.SetBuffer(noiseData.m_Data.data(), noiseData.m_Data.size(), static_cast<int>(noiseData.m_Size.x), static_cast<int>(noiseData.m_Size.y), LAG::TextureFormat::FORMAT_R);
		texture.Reload();
