...
#ifdef DEBUG
	#define LAG_GRAPHICS_CHECK(hResult) if(FAILED(hResult)) throw GraphicsException(__LINE__, __FILE__, HRESULT(hResult) );
...
#else
	#define LAG_GRAPHICS_CHECK(hResult) hResult
